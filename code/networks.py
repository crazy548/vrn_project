...
def discriminator(param):
    ...
    x_src_pose = Input(shape=( int(img_h / pose_dn) , int(img_w / pose_dn), n_joints))
    x_tgt_pose = Input(shape=( int(img_h / pose_dn) , int(img_w / pose_dn), n_joints))
...
def gan(gen_model, disc_model, param):
    ...
    pose_src = Input(shape=( int(img_h / pose_dn) , int(img_w / pose_dn) , n_joints))
    pose_tgt = Input(shape=( int(img_h / pose_dn) , int(img_w / pose_dn), n_joints ))
